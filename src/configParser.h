...
std::vector<cv::Scalar> cocoColors = {
    {0.86f, 0.08f, 0.24f},
    {0.47f, 0.04f, 0.13f},
    {0.00f, 0.00f, 0.56f},
    {0.00f, 0.00f, 0.90f},
...
public:
    ...
    std::vector<ITBBoxTransformPtr> aTargetPostTransforms;
    std::shared_ptr<Transforms::FilterBBoxes> aTargetsFilterTransform;  // Special case of transforms for list of bbox
    std::shared_ptr<Transforms::NMSBBoxes> aTargetsNMSTransform;  // Special case of transforms for list of bbox
    Model aModel;
    std::vector<std::string> aLabels = cocoLabels;
    std::vector<cv::Scalar> aColors = cocoColors;
private:
    std::vector<ITImgTransformPtr> mParsePreProcessing(cv::FileStorage inputFs);
    std::vector<ITBBoxTransformPtr> mParsePostProcessing(cv::FileStorage inputFs);
    Model mParseModel(cv::FileStorage inputFs);
    std::vector<std::string> mParseLabels(cv::FileStorage inputFs);
    std::vector<cv::Scalar> mParseColors(cv::FileStorage inputFs);
};
